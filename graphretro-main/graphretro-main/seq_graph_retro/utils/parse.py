...
def get_reaction_core(r: str, p: str, kekulize: bool = False, use_h_labels: bool = False) -> Tuple[Set, List]:
    ...
    prod_bonds = get_bond_info(prod_mol)  # 产物的键
    p_amap_idx = {atom.GetAtomMapNum(): atom.GetIdx() for atom in prod_mol.GetAtoms()}  # 产物的原子编码：原子排列数子
...
def get_reaction_info(rxn_smi: str, kekulize: bool = False, use_h_labels: bool = False,
    ...
    contains information on the reaction core(反应中心), core edits（核心编辑）, added leaving groups（离去基团）
    and attaching atoms.（附加原子--离去基团）
    ...
    rxn_core, core_edits = get_reaction_core(r, p, kekulize=kekulize, use_h_labels=use_h_labels)  # 获取反应中心 与 核心编辑
    ...
    attach_atoms = get_attach_atoms(rxn_smi=rxn_smi, core_edits=core_edits, core=rxn_core)  # 输入是反应式、核心编辑、反应中心
